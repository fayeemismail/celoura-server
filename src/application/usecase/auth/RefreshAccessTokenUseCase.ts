...
export class RefreshAccessTokenUseCase implements IRefreshAccessTokenUseCase {
    ...
    async execute(refreshToken: string): Promise<{ accessToken: string; userId: string; }> {
        ...
        const payload = this._authService.verifyRefreshToken(refreshToken);
        ...
        const user = await this._userRepo.getUserById(payload.id);

        if (!user) throw new Error('User not found');

        const newAccessToken = this._authService.generateAccessToken({ id: user._id, role: user.role });
